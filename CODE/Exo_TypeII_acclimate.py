...
#Reduce number of traces for clarity
Ts=['2300','2600','2800','5800']
colours=['maroon','red','darkorange','darkblue']

filename='Scaled_Spectrum_PHOENIX_'
N_LHC=np.arange(1.0,16.0,1.0) #range of antenna subunits
...
for Temp in Ts: #loop over stellar temperatures
    ...
    for n in N_LHC:
        ...
        model=thermo.antenna_TypeII(l,irad_y,G_params,Size_params,k_params,T)
        if model['nu_e']<=100.0:            
...
plt.savefig('Exo_TypeII_nu_acclimate.pdf')
plt.show()
